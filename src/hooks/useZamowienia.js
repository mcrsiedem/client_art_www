...
export function useZamowienia() {
  ...
  const {pagination,updatePagination,widok,nr,rok,klient,praca,isbn,kod,nr_zamowienia_klienta,nr_kalkulacji,papierId,wysokosc,szerokosc} = useContext(ZamowienieContext);
  ...
  const scrollTable = (table) => {
    ...
    let  res = await axios.post(
      IP +
        ...
        sessionStorage.getItem("token"),{ nr: nr, rok: rok, praca: praca, klient: klient,isbn:isbn, kod_pracy:kod,nr_zamowienia_klienta ,nr_kalkulacji,papier_id:papierId,format_x:wysokosc,format_y:szerokosc}
