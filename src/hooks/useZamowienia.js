...
export function useZamowienia() {
  ...
  const {pagination,updatePagination,widok,nr,rok,klient,praca,isbn,kod,nr_zamowienia_klienta,nr_kalkulacji,papierId,wysokosc,szerokosc,element_proces_id,arkusz_wysokosc,arkusz_szerokosc,dataSpedycji,dataPrzyjecia} = useContext(ZamowienieContext);
  ...
  const scrollTable = (table) => {
    ...
    let  res = await axios.post(
      IP +
        ...
        sessionStorage.getItem("token"),{ nr: nr, rok: rok, praca: praca, klient: klient,isbn:isbn, kod_pracy:kod,nr_zamowienia_klienta ,nr_kalkulacji,papier_id:papierId,format_x:szerokosc,format_y:wysokosc,element_proces_id,arkusz_wysokosc,arkusz_szerokosc,dataSpedycji,dataPrzyjecia}
