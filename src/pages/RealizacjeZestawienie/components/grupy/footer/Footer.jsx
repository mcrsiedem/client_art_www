  ...
  return (
    <footer onDoubleClick={()=>{  
      ...
      <div className={style.centerHeaderContener}>
        ...
        <div className={style.title_przeloty_container}> <p className={style.title_przeloty}> Narządy: </p><p className={style.title_przeloty_wartosc}> { contextApp.realizacjeZestawienieGrupy.map(x => parseInt(x.LiczbaWpisow)).reduce((a, b) => a + b, 0).toLocaleString()} </p></div>           
        <div className={style.title_przeloty_container}> <p className={style.title_przeloty}> Przeloty: </p><p className={style.title_przeloty_wartosc}> { contextApp.realizacjeZestawienieGrupy.map(x => parseInt(x.SumaZrealizowanoTyp1)).reduce((a, b) => a + b, 0).toLocaleString()} </p></div>           
      </div>
