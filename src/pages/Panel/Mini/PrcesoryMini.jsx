...
setGrupWykonanAllNiezakonczoneOprawa(res.data[1])
  ...
  useEffect(() => {
    checkToken();
        ...
        {DecodeToken(sessionStorage.getItem("token")).mini_falc==1 && (grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 20 ).length !=0) ? <div className={style.kafle} onClick={() => {setSelectedProcesor(20); navigate("/ProcesyMini") }} ><p className={style.znak }>  </p><img className={style.icon } src={iconProcesy} alt="Zamówienia" /><p className={style.menu_txt}>STAHL_1</p><p className={style.menu_txt_zielony} > {grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 20 && (x.status ==2 || x.status ==3) ).length}</p> </div>:<></>}                
        ...
        {DecodeToken(sessionStorage.getItem("token")).mini_falc==1 && grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 14 ).length !=0? <div className={style.kafle} onClick={() => {setSelectedProcesor(14); navigate("/ProcesyMini") }} ><p className={style.znak }>  </p><img className={style.icon } src={iconProcesy} alt="Zamówienia" /><p className={style.menu_txt}>STAHL_6</p> <p className={style.menu_txt_zielony} > {grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 14 && (x.status ==2 || x.status ==3) ).length}</p></div>:<></>}                
        {DecodeToken(sessionStorage.getItem("token")).mini_falc==1 && grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 19 ).length !=0? <div className={style.kafle} onClick={() => {setSelectedProcesor(19); navigate("/ProcesyMini") }} ><p className={style.znak }>  </p><img className={style.icon } src={iconProcesy} alt="Zamówienia" /><p className={style.menu_txt}>STAHL_7</p> <p className={style.menu_txt_zielony} > {grupyWykonanAllNiezakonczone.filter(x=> x.procesor_id == 19 && (x.status ==2 || x.status ==3) ).length}</p></div>:<></>}                
