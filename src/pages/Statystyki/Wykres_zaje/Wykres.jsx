...
const Wykres = () => {
  ...
  // Filtrowanie konfiguracji na podstawie wybranej zakładki
  ...
  const styles = {
    ...
    controlsRow: { display: 'flex', gap: '20px', marginBottom: '2rem', alignItems: 'center' },
    ...
    yAxis: { position: 'absolute', left: 0, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', fontSize: '12px', color: '#94a3b8', fontWeight: '600', paddingBottom: '20px' },
  ...
  const processedData = useMemo(() => {
    ...
    daneZamowieniaProgres.forEach(item => {
      ...
      if (!groups[key]) {
        groups[key] = { 
          label: key, range, timestamp: date.getTime(),
  ...
  // Skala Y teraz liczy się tylko dla aktywnych (widocznych) słupków
  const maxVal = useMemo(() => {
    const allVisibleValues = processedData.flatMap(g => 
      activeConfig.map(c => g.values[c.key])
    );
    const peak = Math.max(...allVisibleValues, 100);
    // Dynamiczny krok zaokrąglania (mniejszy dla mniejszych wartości)
    const step = peak > 50000 ? 10000 : 5000;
    return Math.ceil(peak / step) * step;
  }, [processedData, activeConfig]);
      ...
      <div style={styles.wrapper}>
        <div style={styles.header}>
          <div>
            ...
            <p style={{ color: '#64748b', margin: '5px 0 0 0' }}>Analiza obciążenia procesów</p>
          </div>
        </div>
        ...
        <div style={styles.controlsRow}>
          ...
          <div style={styles.buttonGroup}>
            ...
            <button onClick={() => setProcessTab('finishing')} style={styles.navButton(processTab === 'finishing', '#f59e0b')}>Oprawa (PUR/HM/Zesz)</button>
          </div>
